<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateInstalacionTagProyectoTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('instalacion_tag_proyecto', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('instalacion_id');
            $table->unsignedBigInteger('proyecto_id');

            $table->foreign('instalacion_id')->references('id')
                ->on('instalacion_tags')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('proyecto_id')->references('id')
                ->on('proyectos')->onUpdate('cascade')->onDelete('cascade');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('instalacion_tag_proyecto');
    }
}
