...
@section('content')
    <section class="content">
                    ...
                    <div class="card">
                        <div class="card-header d-flex">
                            ...
                            @can('admin.facilidad.create')
                                <button type="button" class="btn btn-sm btn-success ml-auto" data-toggle="modal" data-target="#crearinstalacion">
                            ...
                            <table id="dataTable" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>instalaciones</th>
                                        <th>estatus</th>
                                        @canany(['admin.instalacion.edit', 'admin.instalacion.destroy'])
                                        <th>Action</th>                       
                                        @endcan
                                    </tr>
                                </thead>
                                <tbody>
                                    @if($rows)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    @can('admin.instalacion.edit')
                                                    <a href="#" class="btn btn-info btn-flat" id="edit-customer" data-toggle="modal" data-id="{{ $row->id }}">
                                                        <i class="far fa-eye"></i>
                                                    </a>                
                                                    @endcan
                                                    @can('admin.instalacion.destroy')
                                                    <form action="{{route('admin.instalacion.destroy', $row->id)}}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
        ...
        @include('admin.instalaciones.modal')
    </section>
@stop
...
@section('js')
    ...
    <script>
        ...
        $('body').on('click', '#edit-customer', function () {
            ...
            $.get('Instalacion/'+row_id+'/edit', function (data) {
                console.log(data.estatus)
                $('#customerCrudModal').html("Modificar Tag de instalaciones");
                $('#btn-update').val("Update");
                $('#btn-save').prop('disabled',false);
                $('#editarinstalacion').modal('show');
                $('#cust_id').val(data.id);
